...
export function AiChat({ authenticated, guestSessionId, guestUsed, onGuestUsed, onRequireLogin, onClose, onApplyBuild }: {
  ...
  return <div className="modal-backdrop ai-backdrop" onMouseDown={onClose}><section className="ai-chat" role="dialog" aria-modal="true" aria-label="NexRig AI builder" onMouseDown={(e) => e.stopPropagation()}>
    <aside className="ai-history"><div><span className="brand-mark">N</span><b>NEXRIG AI</b></div>{authenticated ? <><button onClick={() => { setConversationId(undefined); setMessages([]); }}>+ New build chat</button><nav>{conversations.map((conversation) => <div className={`ai-conversation-row ${conversation.id === conversationId ? "active" : ""}`} key={conversation.id}><button onClick={() => void openConversation(conversation.id)}><b>{conversation.title}</b><small>{conversation.preview}</small></button><button className="ai-delete-chat" aria-label={`Delete ${conversation.title}`} onClick={() => void deleteConversation(conversation.id)}>×</button></div>)}</nav></> : <div className="guest-note"><b>Guest mode</b><span>One prompt per page load. Sign in for history and follow-ups.</span><button onClick={onRequireLogin}>Sign in</button></div>}</aside>
    <div className="ai-main"><header><div><div className="step">DATABASE-GROUNDED RAG</div><h2>Build with NexRig AI</h2></div><button aria-label="Close" onClick={onClose}>×</button></header>
      <div className="ai-messages">{messages.length === 0 && <div className="ai-empty"><span>✦</span><h3>What do you want to build?</h3><p>Include your budget, games or software, resolution and target FPS.</p><button onClick={() => setInput("Build me a GTA V gaming PC under ৳100,000 for 1080p 60 FPS")}>Try a GTA V build</button></div>}
        {messages.map((message, index) => <article className={`ai-message ${message.role}`} key={`${message.role}-${message.id ?? `local-${index}`}`}><div className="ai-role">{message.role === "user" ? "YOU" : "NEXRIG AI"}</div><StructuredAnswer content={message.content} />
          {message.build && message.build.length > 0 && <div className="ai-build"><header><b>VALIDATED DATABASE BUILD</b><button onClick={() => onApplyBuild(message.build!)}>Apply to builder</button></header>{message.build.map((product) => <div key={product.id}><span>{product.category}</span><strong>{product.name}</strong><b>৳{product.price.toLocaleString("en-BD")}</b></div>)}</div>}
